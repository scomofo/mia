// plan-screen.jsx — refinement + generated meal plan preview

const { useState: useStateP, useEffect: useEffectP } = React;

// Derived profile summary from answers
function deriveProfile(a) {
  ...
  // Rough calorie estimate if we have body data
  let calories = null;
  if (a.body && a.body.w && a.age) {
    const w = a.body.w;
    const sex = a.body.sex;
    // Very simplified Mifflin-St Jeor using 70" default height
    const bmr = sex === 'male' ? 10 * (w/2.2) + 6.25 * 178 - 5 * a.age + 5
...
function RefineScreen({ prompt, answers, onGenerate, onBack }) {
  const profile = deriveProfile(answers);
  const [cals, setCals] = useStateP(profile.calories || 2200);
  const [protein, setProtein] = useStateP(160);
    ...
    <div style={{ height: '100%', overflow: 'auto', background: 'var(--cream)' }}>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          }}>Step 3 of 3 · Refine</div>
        </div>
      </div>

      <div style={{ padding: '0 16px 140px' }}>
        {/* Selected prompt pill */}
        <div style={{
          ...
          <window.PromptIcon id={prompt.id} size={48} />
          <div style={{ flex: 1 }}>
            <div style={{
            ...
            <div style={{
              ...
              color: 'var(--ink)', letterSpacing: -0.01, lineHeight: 1.1, marginTop: 2,
            }}>{prompt.title}</div>
          </div>
        </div>
        ...
        <h2 style={{
          ...
          letterSpacing: -0.02, margin: '8px 0 14px', lineHeight: 1.05,
        }}>Fine-tune before <em style={{ color: 'var(--olive-deep)' }}>generating</em></h2>

        {/* Profile summary */}
        <div style={{
          background: 'var(--cream-2)', borderRadius: 18, padding: 16, marginBottom: 16,
        }}>
          <div style={{
            fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-3)',
        ...
        {/* What you'll get */}
        <div style={{
          background: 'var(--olive-soft)', borderRadius: 18, padding: 16, marginBottom: 16,
        }}>
          <div style={{
            fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--olive-deep)',
            letterSpacing: 0.08, textTransform: 'uppercase', marginBottom: 8, fontWeight: 500,
          }}>You'll get</div>
      ...
      <div style={{
        ...
        <button onClick={() => onGenerate({ cals, protein })}
          style={{
...
// ─── Plan preview screen (after generation) ───────────────
...
function PlanPreviewScreen({ prompt, answers, tuning, onRestart, onBack, onNav }) {
  const [dayIdx, setDayIdx] = useStateP(0);
  const [liveDays, setLiveDays] = useStateP(null);
  const [status, setStatus] = useStateP('idle'); // idle | loading | done | error
  const [summary, setSummary] = useStateP('');

  React.useEffect(() => {
    let cancelled = false;
    async function run() {
      if (!window.generatePlan || !window.claude) return;
      setStatus('loading');
      try {
        const plan = await window.generatePlan(answers, prompt, tuning);
        if (cancelled) return;
    ...
    <div style={{ height: '100%', overflow: 'auto', background: 'var(--cream)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          ...
          <div style={{
            fontFamily: 'var(--mono)', fontSize: 10.5, letterSpacing: 0.08,
            textTransform: 'uppercase', opacity: 0.7,
          }}>Your plan · generated just now</div>
        </div>
        <h1 style={{
          fontFamily: 'var(--serif)', fontSize: 36, fontWeight: 400,
          lineHeight: 1, margin: 0, letterSpacing: -0.02,
        }}>
          Your <em style={{ color: 'oklch(90% 0.1 130)' }}>7-day</em><br/>meal plan
        </h1>
        <div style={{
          display: 'flex', gap: 14, marginTop: 18,
          fontFamily: 'var(--mono)', fontSize: 11, opacity: 0.85,
        }}>
          <div>
            <div style={{ opacity: 0.7 }}>Target</div>
            <div style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 20, marginTop: 2 }}>
              {tuning.cals} kcal
            </div>
          </div>
          <div>
        ...
        {status === 'loading' && (
          <div style={{
            ...
            <div style={{
              fontFamily: 'var(--sans)', fontSize: 13, opacity: 0.9, letterSpacing: -0.1,
            }}>Mia is cooking up your week…</div>
          </div>
        )}
        {status === 'done' && summary && (
          <div style={{
      ...
      {/* Day switcher */}
      <div style={{
        padding: '16px 12px 6px', display: 'flex', gap: 6, overflowX: 'auto',
      }}>
        {days.map((d, i) => (
          <button key={d.day} onClick={() => setDayIdx(i)} style={{
            ...
            <span style={{
              fontFamily: 'var(--mono)', fontSize: 9.5, letterSpacing: 0.08,
              opacity: dayIdx === i ? 0.7 : 0.55,
            }}>{d.day}</span>
            <span style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: 'var(--ink)', letterSpacing: -0.01,
        }}>
          ...
          <div key={i} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{
                ...
                color: 'var(--ink)', lineHeight: 1.3, letterSpacing: -0.1,
              }}>{m.name} {m.kid && <span style={{
                fontFamily: 'var(--script)', fontSize: 15, color: 'var(--tangerine)',
              }}>kid-friendly</span>}</div>
              <div style={{
...
window.PlanPreviewScreen = PlanPreviewScreen;
