// Builds user profile text + the Claude request for 7-day plan JSON.

export function buildProfile(answers, tuning) {
  const L = [];
...
export function buildPlanRequest(answers, prompt, tuning) {
  const profile = buildProfile(answers, tuning);
  return `You are a senior nutrition coach (Nav Toor style prompt: "${prompt.title}").
...
- Meals should reflect loves/hates and restrictions
