...
import { CHAT_FLOW } from './data/chat-flow';
import { rankPrompts } from './data/prompts';
    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{
          ...
          Mia
          <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--olive)', marginLeft: 2 }} />
        </div>
        <div style={{
          fontFamily: 'var(--mono)', fontSize: 10.5,
          color: 'var(--ink-3)', letterSpacing: 0.04, marginTop: 1,
        }}>dietitian AI · {step}/{total}</div>
      </div>
      <div style={{
        fontFamily: 'var(--mono)', fontSize: 10.5, color: 'var(--ink-3)',
        padding: '4px 8px', borderRadius: 6, background: 'rgba(31,36,25,0.04)',
      }}>meal-plan-01</div>
    </div>
  );
}
...
function ProgressStrip({ pct }) {
  return (
    <div style={{ height: 3, background: 'rgba(31,36,25,0.06)', position: 'relative' }}>
      <div style={{ height: '100%', width: `${pct}%`, background: 'var(--olive)', transition: 'width 400ms cubic-bezier(0.4,0,0.2,1)' }} />
    </div>
  );
}
...
function QuickReply({ options, onPick }) {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, padding: '2px 12px 12px', justifyContent: 'flex-end' }}>
      {options.map(o => (
        <button key={o.id} onClick={() => onPick(o)} style={{
          background: '#fff', border: '1px solid rgba(31,36,25,0.12)',
          borderRadius: 999, padding: '10px 14px',
          fontFamily: 'var(--sans)', fontSize: 14, fontWeight: 500,
          color: 'var(--olive-deep)', cursor: 'pointer',
          letterSpacing: -0.1, boxShadow: '0 1px 2px rgba(31,36,25,0.04)',
...
function ChipPicker({ options, max, min = 1, onSubmit }) {
  ...
  return (
    <div style={{ padding: '2px 12px 12px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10, gap: 8 }}>
        {max && <span style={{
          fontFamily: 'var(--mono)', fontSize: 10.5, color: 'var(--ink-3)', alignSelf: 'center',
        }}>{sel.length}/{max}</span>}
        <button onClick={() => onSubmit(sel)} disabled={!canSubmit} style={{
          ...
          cursor: canSubmit ? 'pointer' : 'not-allowed', letterSpacing: -0.1,
        }}>Send →</button>
      </div>
    </div>
  );
    ...
    <div style={{ padding: '8px 12px 14px' }}>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 10 }}>
          <span style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 42, lineHeight: 1, color: 'var(--olive-deep)' }}>{val}</span>
          <span style={{ fontFamily: 'var(--mono)', fontSize: 13, color: 'var(--ink-3)' }}>{unit}</span>
        </div>
        <div style={{ position: 'relative', padding: '8px 0 4px' }}>
          <div style={{ height: 6, borderRadius: 999, background: 'rgba(31,36,25,0.08)', position: 'relative' }}>
            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${pct}%`, background: 'var(--olive)', borderRadius: 999 }} />
          </div>
          ...
          <div style={{
            ...
            pointerEvents: 'none', boxShadow: '0 2px 4px rgba(31,36,25,0.1)',
          }} />
        </div>
        {marks && (
          <div style={{
          ...
          }}>{marks.map(m => <span key={m.v}>{m.label}</span>)}</div>
        )}
      </div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
        <button onClick={() => onSubmit(val)} style={{
          ...
          border: 'none', borderRadius: 999, padding: '10px 18px',
          fontFamily: 'var(--sans)', fontSize: 14, fontWeight: 600, cursor: 'pointer', letterSpacing: -0.1,
        }}>Set {val}{unit} →</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: '2px 12px 12px', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
      <div style={{
        ...
        background: '#fff', borderRadius: 999, padding: '4px 4px 4px 16px',
        boxShadow: '0 1px 2px rgba(31,36,25,0.05), 0 0 0 1px rgba(31,36,25,0.06)',
      }}>
        <input type="number" value={val} onChange={e => setVal(e.target.value)}
          placeholder="age" min="13" max="99"
          style={{ border: 'none', outline: 'none', width: 60, fontFamily: 'var(--sans)', fontSize: 15, background: 'transparent', color: 'var(--ink)' }} />
        <button onClick={() => val && onSubmit(+val)} disabled={!val} style={{
    ...
    <div style={{ padding: '2px 12px 12px' }}>
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <label style={{ flex: 1 }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-3)', letterSpacing: 0.08, textTransform: 'uppercase', marginBottom: 3 }}>Height</div>
            <input value={h} onChange={e => setH(e.target.value)}
              style={{ width: '100%', border: '1px solid rgba(31,36,25,0.1)', borderRadius: 8, padding: '8px 10px', fontSize: 14, fontFamily: 'var(--sans)', background: '#FBF8F0' }} />
          </label>
          <label style={{ flex: 1 }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-3)', letterSpacing: 0.08, textTransform: 'uppercase', marginBottom: 3 }}>Weight (lb)</div>
...
export default function ChatApp({ onComplete, answers, setAnswers }) {
  ...
  const questionTurns = useMemo(() => flow.filter(t => t.kind !== 'message' && t.kind !== 'thinking').length, [flow]);
  ...
  useEffect(() => {
    ...
    if (turn.kind === 'message' || turn.kind === 'thinking') {
      ...
      const t1 = setTimeout(() => {
        setTyping(false);
        if (turn.kind === 'message') setVisible(v => [...v, cursor]);
        setCursor(cursor + 1);
      }, delay);
  ...
  }, [cursor, typing]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    if (turn.kind === 'message') {
      return (
        <div key={turn.id} style={{ display: 'flex', alignItems: 'flex-end' }}>
          <div style={{ width: 28, marginRight: 8, flexShrink: 0 }}>{isLastInGroup && <MiaAvatar />}</div>
          <div style={{ flex: 1 }}>
    ...
    return (
      <div key={turn.id}>
        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
          <div style={{ width: 28, marginRight: 8, flexShrink: 0 }}>{!userReplies[turn.id] && <MiaAvatar />}</div>
          <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--cream)' }}>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflow: 'auto', padding: '14px 12px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {visible.map(idx => {
          const turn = flow[idx];
      ...
      <div style={{
        background: 'rgba(244,235,212,0.85)', backdropFilter: 'blur(12px)',
        borderTop: '1px solid var(--divider)', paddingBottom: 34,
      }}>
        {showInput && activeTurn.kind === 'quick-reply' && (
          <QuickReply options={activeTurn.options} onPick={o => handleAnswer(activeTurn, o.reply, o.id)} />
        )}
        {showInput && activeTurn.kind === 'chips' && (
          <ChipPicker options={activeTurn.options} max={activeTurn.max} min={activeTurn.min || 1}
            onSubmit={vals => {
